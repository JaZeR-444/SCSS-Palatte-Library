...
// Projects are prerendered; collections (and promoted workspaces) render
// on-demand via the default dynamicParams behaviour.
...
export async function generateMetadata({
  ...
  return {
    ...
    description: name
      ? `Palettes and design systems for ${name}.`
      : "Workspace palettes.",
  };
}
...
export default async function WorkspaceDetailPage({
  ...
  const palettes = getProjectPalettes(slug);
  const meta = getProjectMeta(slug) ?? { type: "Product", description: "" };
  const manualIds = getManualPaletteIds(slug);
  const presets = getProjectPresets(slug);
  const designSystems = listDesignSystems(slug);
