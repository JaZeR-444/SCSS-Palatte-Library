...
import { Heart, MessageCircle, Repeat2, Send, CheckCircle2, UserPlus, UserCheck } from "lucide-react";
    ...
    <div className="w-full max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        >
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <button 
              ...
              style={{ 
                backgroundColor: followed ? roleMapping["--ui-color-5"] : roleMapping["--ui-color-1"],
                color: followed ? roleMapping["--ui-color-6"] : roleMapping["--ui-color-4"]
              }}
            >
              {followed ? (
                <>
          ...
          <div className="space-y-4">
            ...
            >
              Experimenting with our new color systems library. The palette values feel incredibly rich when mapped to interfaces! How does this look? {" "}
              <span 
            ...
            >
              ...
              <div className="relative z-10 bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-xl text-white max-w-sm">
                <p className="text-[10px] font-black uppercase tracking-widest text-white/70">Palette Preview</p>
                <h4 className="font-bold text-sm leading-tight mt-1">Linear Gradient Mesh Simulation</h4>
              </div>
            </div>
            ...
            >
              <div className="flex gap-6">
                ...
                <button 
                  onClick={toggleShare}
                  className="flex items-center gap-2 text-xs font-bold transition-colors group"
                  style={{ color: shared ? roleMapping["--ui-color-8"] : roleMapping["--ui-color-7"] }} // Success State
                >
                ...
                <button 
                  onClick={toggleLike}
                  className="flex items-center gap-2 text-xs font-bold transition-colors group"
                  style={{ color: liked ? roleMapping["--ui-color-10"] : roleMapping["--ui-color-7"] }} // Danger State (Red)
                >
                  <Heart className={`h-4 w-4 group-hover:scale-125 transition-transform ${liked ? 'fill-current' : ''}`} />
                  <span>{likeCount}</span>
                </button>
              </div>
      ...
      <div className="lg:col-span-4 flex flex-col gap-4">
        ...
        >
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold" style={{ color: roleMapping["--ui-color-6"] }}>Active Users</span>
              <span 
                className="text-xs font-black px-2 py-0.5 rounded-full"
                style={{ backgroundColor: roleMapping["--ui-color-8"] + "22", color: roleMapping["--ui-color-8"] }}
              >
                +18%
              </span>
            </div>
            ...
            <div className="flex items-center justify-between pt-2">
              <span className="text-xs font-bold" style={{ color: roleMapping["--ui-color-6"] }}>Server Status</span>
              <span className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider" style={{ color: roleMapping["--ui-color-8"] }}>
        ...
        >
          ...
          <p className="text-[11px] leading-relaxed font-medium" style={{ color: roleMapping["--ui-color-7"] }}>
            Verifying avatar surfaces against primary background and hover state changes for interactive components.
          </p>
        </div>
      </div>
    </div>
